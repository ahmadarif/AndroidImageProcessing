...
public class AverageFilter {
    ...
    private static int kernal_width = 3;
    private static int kernal_height = 3;
    private static int kernal_halfWidth = 1;
    private static int kernal_halfHeight = 1;

    private static Bitmap convBitmap(Bitmap src) {
            ...
            for (int y = 0; y < h; y++) {
                ...
                for (int k = 0; k < kernal_width; k++) {
                    for (int l = 0; l < kernal_height; l++) {
                        pixelR += Color.red(sourceMatrix[k][l]) * matrix[k][l];
                        pixelG += Color.green(sourceMatrix[k][l]) * matrix[k][l];
                        pixelB += Color.blue(sourceMatrix[k][l]) * matrix[k][l];
                        pixelA += Color.alpha(sourceMatrix[k][l]) * matrix[k][l];
                    }
                }
                ...
                bm.setPixel(x, y, Color.argb(pixelA, pixelR, pixelG, pixelB));
            }
        }
    ...
    public static Bitmap process(Bitmap bitmap, int loop) {
        int idx = 0;
        do {
